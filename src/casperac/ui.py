...
class SudoDialog(ctk.CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self,
            text="⚠️ Yetkilendirme Gerekiyor",
        ...
        self.info = ctk.CTkLabel(
            self,
            text="Kill-Switch gibi sistem ağını koruyan derin\nmüdahaleler için Mac/Linux yönetici şifreniz gereklidir.",
            justify="center",
        )
        self.info.pack(pady=5)

        self.pwd_entry = ctk.CTkEntry(
            self, show="*", width=200, placeholder_text="Yönetici Şifresi (Sudo)"
        )
...
class CasperWindow(ctk.CTk):
    def __init__(self):
        ...
        self.tor_status = ctk.CTkLabel(
            self.status_frame, text="Tor: Checking...", text_color="yellow"
        )
        self.tor_status.grid(row=0, column=0, padx=10)

        self.vpn_status = ctk.CTkLabel(
            self.status_frame, text="VPN: Checking...", text_color="yellow"
        )
        ...
        self.renew_btn = ctk.CTkButton(
            self,
            text="Renew IP Identity",
            command=self.renew_ip,
            fg_color="#8a2be2",
            hover_color="#5a189a",  # Purple colors
        )
    ...
    def change_country(self, choice):
        ...
        self.log(
            f"Çıkış ülkesi {choice} olarak ayarlanıyor, Tor yeniden başlatılacak..."
        )
        threading.Thread(
            target=self._change_country, args=(country_code,), daemon=True
        ).start()
